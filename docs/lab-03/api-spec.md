...
The session store is server-side and keyed by a random opaque cookie; sessions expire after a documented inactivity/absolute lifetime selected by implementation configuration. The Lab implementation stores records in an in-memory `Map`, so restarting the server logs out all users; this is not suitable for production or multi-instance deployment. Logout destroys the server record and clears the cookie. Cookies are `HttpOnly`, `SameSite=Lax`, and `Secure` when HTTPS is enabled. Set `SESSION_COOKIE_SAMESITE=None` only when the frontend and backend are genuinely cross-site; `SameSite=None` requires `Secure` and therefore HTTPS. Different localhost ports are still same-site, so the default `Lax` policy is appropriate for the local Docker setup. SameSite plus an Origin/CSRF-token check protects state changes. Initial-password behavior is local-lab only and never sends credentials through email. Secrets and session keys come from environment configuration and are never committed.
