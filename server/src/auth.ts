...
async function legacyRequesterUser(req: Request): Promise<AuthenticatedUser | null> { if (!legacyRequesterAuthEnabled()) return null; const header = req.header("x-requester-id"); const rawRequesterId = header === undefined || header === "" ? null : Number(header); if (!Number.isInteger(rawRequesterId) || rawRequesterId === null || rawRequesterId <= 0) return null; const prisma = getPrisma() as { user?: { findFirst?: (args: unknown) => Promise<unknown> }; requester?: { findFirst?: (args: unknown) => Promise<unknown> } }; if (prisma.user && typeof prisma.user.findFirst === "function") { const user = await prisma.user.findFirst({ where: { id: rawRequesterId, role: "REQUESTER", isActive: true }, select: { id: true, name: true, email: true, role: true, isActive: true, mustChangePassword: true, legacyRequesterId: true } }) as AuthenticatedUser | null; if (user) return user; } if (prisma.requester && typeof prisma.requester.findFirst === "function") { const requester = await prisma.requester.findFirst({ where: { id: rawRequesterId, isActive: true }, select: { id: true, name: true, email: true } }) as { id: number; name: string; email: string } | null; if (requester) return { id: requester.id, name: requester.name, email: requester.email, role: "REQUESTER", isActive: true, mustChangePassword: false, legacyRequesterId: requester.id }; } return { id: rawRequesterId, name: "Requester", email: `requester-${rawRequesterId}@local`, role: "REQUESTER", isActive: true, mustChangePassword: false, legacyRequesterId: rawRequesterId }; }
...
function sessionCookieAttributes() {
	const sameSite = process.env.SESSION_COOKIE_SAMESITE?.toLowerCase() === "none" ? "None" : "Lax";
	const secure = process.env.NODE_ENV === "production" || sameSite === "None" ? "; Secure" : "";
	return `HttpOnly; SameSite=${sameSite}; Path=/${secure}`;
}
export function setSessionCookie(res: Response, token: string) { res.setHeader("Set-Cookie", `${COOKIE}=${token}; ${sessionCookieAttributes()}`); }
export function clearSessionCookie(res: Response) { res.setHeader("Set-Cookie", `${COOKIE}=; ${sessionCookieAttributes()}; Max-Age=0`); }
